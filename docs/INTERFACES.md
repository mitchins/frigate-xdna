...
| `fxdna status [REF] [--json]` | Show model/job/service state and active worker without opening the NPU. |
| `fxdna wait REF [--state prepared\|verified\|active] [--timeout SECONDS]` | Wait through private admin socket; default `prepared`, timeout 1800 s. Fail immediately on terminal error/quarantine. Never claim prepared means resident. |
| `fxdna activate REF [--maintenance] [--wait]` | Activate an already prepared model. Changing a busy active model requires `--maintenance`; new child, no hot swap. No implicit download/compile here. |
...
| `fxdna health [--ready]` | Supervisor liveness by default; `--ready` requires an ACTIVE healthy worker. No NPU probe. |
